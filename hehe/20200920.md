...
答：可以是可以，不错这就放弃了 tree-shaking，毕竟 **理解代码** 是优化的终点。
就好比当初只要一个 cdn 地址，jquery/lodash 所有方法随心用，到如今，每个库/框架都要小心翼翼保证 tree-shakable (lighthouse 这玩意儿可怕哟)。

## vercel/vercel

众所周知，vercel 是 next.js 背后的 👩，配置好 GitHub 之后只要 push，剩下的打包发布上线都由 vercel 完成。这个 "完成" 背后，就有 vercel runtime 的参与。
...
等等等等，说到 vercel runtime，顺便得提一句，事实上 vercel 不光支持 node/next，也支持其他语言 (go/python/ruby ...)，长话短说：只要能在 lambda 上跑，管你什么语言。

vercel 对其平台的 runtime 定义是：并符合 Runtime interface 的 node package ([DEVELOPING_A_RUNTIME.md](https://github.com/vercel/vercel/blob/master/DEVELOPING_A_RUNTIME.md))：
...
其主要代码位于 `now-next/src/index.ts`，按 runtime 要求，export build + prepareCache function，其代码执行流程如下:
...
那拿到 output 和 routes 等数据之后，怎么做，**可以** 怎么做？这似乎已经不在本文讨论的范围了。
